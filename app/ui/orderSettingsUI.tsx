...
export default function OrderSettingsUI({
  sortBy,
  sortOrder,
  onSortByChange,
  onOrderChange,
  ...
  sortBy: SortBy;
  sortOrder: SortOrder;
  onSortByChange: (targetSort: SortBy) => void;
  onOrderChange: (targetOrder: SortOrder) => void;
}) {
  return (
    <div className="flex gap-2 h-10 w-full">
      ...
      transition-all duration-200
    ">
        {sortOrder === "asc" ? <BarsArrowUpIcon className="w-8 h-2/3 text-muted-foreground" /> : <BarsArrowDownIcon className="w-8 h-2/3 text-muted-foreground" />}
      </button>
    </div>
  );
}
