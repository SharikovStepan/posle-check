    ...
    <div className={`overflow-hidden relative p-2 rounded-2xl bg-surface h-20 flex gap-3 justify-between items-center transition-all duration-200 border border-border shadow-md`}>
      <div className="h-full grid grid-cols-[auto_1fr_80_auto] gap-x-1 w-full">
        ...
        <div className="flex flex-col justify-between items-start h-full">
          ...
          <p className={`${isCustomAmount ? "text-success" : "text-warning"} opacity-80 text-xs justify-self-end`}>{isCustomAmount ? "Вы ввели сумму" : "Выберет сумму сам"}</p>
        </div>
        ...
        <div className="relative h-full w-20">
          <p
            className={`absolute h-full w-full top-0 text-center flex justify-center items-center text-sm transition-all duration-300 ${
              isCustomAmount ? "-translate-y-[calc(100%+10px)]" : "translate-y-0"
            }`}>
            Ввести сумму
          </p>

          <div
            className={`absolute top-0 w-20 h-full flex flex-col justify-between items-center gap-1 transition-all duration-300 ${isCustomAmount ? "translate-y-0" : "translate-y-[calc(100%+10px)]"}`}>
